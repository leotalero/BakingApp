...
public class BakingProvider  extends ContentProvider {
    ...
    private static final int RECIPE_ = 100;
    ...
    private static UriMatcher buildUriMatcher(){
        ...
        matcher.addURI(authority, BakingContract.BakingEntry.TABLE_NAME, RECIPE_);
        matcher.addURI(authority, BakingContract.BakingEntry.TABLE_NAME + "/#",RECIPE_WITH_ID);
    ...
    public String getType(Uri uri){
        ...
        switch (match){
            case RECIPE_:{
                return BakingContract.BakingEntry.CONTENT_DIR_TYPE;
            }
            case RECIPE_WITH_ID:{
                return BakingContract.BakingEntry.CONTENT_ITEM_TYPE;
            }
            default:{
    ...
    public Uri insert(Uri uri, ContentValues values){
        ...
        switch (sUriMatcher.match(uri)) {
            case RECIPE_: {
                long _id = db.insert(BakingContract.BakingEntry.TABLE_NAME, null, values);
        ...
        switch(match){
            case RECIPE_:
                numDeleted = db.delete(
                        BakingContract.BakingEntry.TABLE_NAME, selection, selectionArgs);
                // reset _ID
                db.execSQL("DELETE FROM SQLITE_SEQUENCE WHERE NAME = '" +
                        BakingContract.BakingEntry.TABLE_NAME + "'");
                break;
            case RECIPE_WITH_ID:
                numDeleted = db.delete(BakingContract.BakingEntry.TABLE_NAME,
                        BakingContract.BakingEntry.COLUMN_RECIPE_ID + " = ?",
                        new String[]{String.valueOf(ContentUris.parseId(uri))});
                // reset _ID
                db.execSQL("DELETE FROM SQLITE_SEQUENCE WHERE NAME = '" +
            ...
            case RECIPE_:
                ...
                try{
                    for(ContentValues value : values){
                        ...
                        long _id = -1;
                        try{
                            _id = db.insertOrThrow(BakingContract.BakingEntry.TABLE_NAME,
                                    null, value);
